  ...
  group('HolderBindingVerifier', () {
    ...
    test(
      ...
      () async {
        final verifier = HolderBindingVerifier();
        final vp = UniversalPresentationParser.parse(
          VerifiablePresentationDataFixtures.vpWithMismatchingCredentialSubject,
        );
        final result = await verifier.verify(vp);
        expect(result.isValid, false);
        expect(result.errors.length, 1);
    ...
    test(
      'should fail when VC has no holder and no credentialSubject.id',
      () async {
        final verifier = HolderBindingVerifier();
        final vp = UniversalPresentationParser.parse(
    ...
    test(
      'should fail if VC is not delegated and holder does not match',
      () async {
        final verifier = HolderBindingVerifier();
        final vp = UniversalPresentationParser.parse(
