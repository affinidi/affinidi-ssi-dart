...
void main() {
  group('DelegationVcVerifier', () {
    ...
    test(
      'should fail if VP DID and VC DID do not match and no delegation VC',
      () async {
        final verifier = DelegationVcVerifier();
        final vp = UniversalPresentationParser.parse(
    ...
    test(
      'should pass if restricted delegationLevel with all VCs matching',
      () async {
        final verifier = DelegationVcVerifier();
        final vp = UniversalPresentationParser.parse(
    ...
    test(
      'should fail if the signer of DelegationVC is not a VP holder',
      () async {
        final verifier = DelegationVcVerifier();
        final vp = UniversalPresentationParser.parse(
    ...
    test(
      'should fail if there are missing vcIds at the DelegationVC',
      () async {
        final verifier = DelegationVcVerifier();
        final vp = UniversalPresentationParser.parse(
        ...
        expect(
          result.errors,
          contains('Missing delegation VC IDs: claimid:vc2'),
        );
      },
    );

    test(
      'should fail if delegationLevel is "restricted" and unexpected VCs id',
      () async {
        final verifier = DelegationVcVerifier();
        final vp = UniversalPresentationParser.parse(
    ...
    test(
      'should verify VP with delegation from DID1 and DID2 to DID3 (full)',
      () async {
        final verifier = DelegationVcVerifier();
        final vp = UniversalPresentationParser.parse(
          VerifiablePresentationDataFixtures.v1VpWithMultiDelegationMixed,
  ...
  test(
    'should pass if V2 VC has no holder and uses credentialSubject.id',
    () async {
      final verifier = DelegationVcVerifier();
      final vp = UniversalPresentationParser.parse(
