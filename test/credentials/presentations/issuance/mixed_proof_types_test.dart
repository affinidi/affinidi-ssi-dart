  ...
  group('Mixed Proof Types in VP', () {
    ...
    setUp(() async {
      ...
      final vpSeed = Uint8List.fromList(
        List.generate(32, (index) => index + 33),
      ); // different seed
      final secp256k1Wallet = Bip32Wallet.fromSeed(vpSeed);
      final secp256k1Key = await secp256k1Wallet.generateKey(
        keyId: "m/44'/60'/0'/0'/0'",
      );
    ...
    test(
      'should create VP with multiple VCs having different proof types',
      () async {
      ...
      () async {
        ...
        if (vcProof is Map<String, dynamic>) {
          ...
          final dataIntegrityProof = vcProof.firstWhere(
            (p) =>
                p is Map<String, dynamic> && p['type'] == 'DataIntegrityProof',
            orElse: () => null,
          );
          expect(dataIntegrityProof, isNotNull);
          expect(
            dataIntegrityProof['@context'],
            equals('https://w3id.org/security/data-integrity/v2'),
          );
        }
    ...
    test(
      'should support ECDSA P-256 DataIntegrityProof VC in secp256k1 VP',
      () async {
        // Create P-256 signer for VC with ECDSA DataIntegrityProof
        final p256Seed = Uint8List.fromList(
          List.generate(32, (index) => index + 65),
        ); // different seed
    ...
    test(
      ...
      () async {
        ...
        final secp256k1VcV1 = await _issueVcV1WithSecp256k1Proof(
          vpSigner,
          vcSigner.did,
        );
        ...
        final vp = MutableVpDataModelV2(
          ...
          type: {'VerifiablePresentation'},
          holder: MutableHolder.uri(
            vcSigner.did,
          ), // VP holder matches VC subject
...
Future<LdVcDataModelV1> _issueVcV1WithSecp256k1Proof(
  ...
  final unsignedVc = MutableVcDataModelV1(
    ...
    credentialSubject: [
      MutableCredentialSubject({'id': subjectDid}),
    ],
  );
