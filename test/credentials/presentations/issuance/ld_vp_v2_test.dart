...
void main() async {
  ...
  final ldV2VC = UniversalParser.parse(
    VerifiableCredentialDataFixtures.credentialWithValidProofDataModelV20String,
  );
  final sdjwtV2VC = UniversalParser.parse(
    VerifiableCredentialDataFixtures.sdJwtWithValidSig,
  );
  ...
  group('VP LD V2 Issuance', () {
    test(
      'should be able to create a presentation containing V2 compatible VCs',
      () async {
        ...
        expect(issuedPresentation.context.firstUri, isNotNull);
        expect(dmV2ContextUrl, isIn(issuedPresentation.context.context));
