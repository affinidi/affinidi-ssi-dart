...
void main() {
  group('DidWebManager', () {
    ...
    setUp(() async {
      ...
      manager = DidWebManager(store: store, wallet: wallet);
    });
    ...
    group('addVerificationMethod', () {
      test('should add verification method', () async {
        ...
        // Act
        final result1 = await manager.addVerificationMethod(keyPair1.id);
        final result2 = await manager.addVerificationMethod(keyPair2.id);

        final verificationMethodId1 = result1.verificationMethodId;
        final verificationMethodId2 = result2.verificationMethodId;

        // Assert
        expect(verificationMethodId1, equals('test-key#1'));
        expect(verificationMethodId2, equals('test-key#2'));
      });
    });
    ...
    group('DID document/controller cross-checks', () {
      test('keyAgreement id in DID document matches keypair in controller',
        ...
        expect(didDocument.id, startsWith('did:web:'));
