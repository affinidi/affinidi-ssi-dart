...
void main() {
  group('DidWebManager', () {
    ...
    setUp(() async {
      ...
      manager = DidWebManager(store: store, wallet: wallet, domain: testDomain);
      await manager.init();
    });
    ...
    group('addVerificationMethod', () {
      ...
      test(
        'should produce unique, stable JWK thumbprint IDs for each key',
        () async {
          final key1 = await wallet.generateKey(
            keyId: 'k1',
            keyType: KeyType.p256,
          ...
          final thumbprintPattern = RegExp(
            r'^did:web:example\.com#[A-Za-z0-9_-]{43}$',
          );
      ...
      test(
        'should derive X25519 key for keyAgreement when using ed25519 key',
        () async {
          final key = await wallet.generateKey(
            keyId: 'ed-key',
            keyType: KeyType.ed25519,
    ...
    group('Mixed key types - DID document structure', () {
      test(
        ...
        () async {
          ...
          expect(
            doc.assertionMethod.first.id,
            assertResult.verificationMethodId,
          );
      ...
      test(
        'signing works with each key type in a mixed-key document',
        () async {
          final authKey = await wallet.generateKey(
            keyId: 'sign-ed',
            keyType: KeyType.ed25519,
    ...
    group('Same key type - multiple keys with shared relationships', () {
      test('2 ed25519 keys: both with authentication + assertionMethod', () async {
        final key1 = await wallet.generateKey(
          keyId: 'ed-1',
          keyType: KeyType.ed25519,
        );
      ...
      test(
        '2 secp256k1 keys: both with assertionMethod + capabilityInvocation',
        () async {
          final key1 = await wallet.generateKey(
            keyId: 'secp-1',
            keyType: KeyType.secp256k1,
          ...
          final vm1mb =
              (doc.verificationMethod[0] as VerificationMethodMultibase)
                  .publicKeyMultibase;
          final vm2mb =
              (doc.verificationMethod[1] as VerificationMethodMultibase)
                  .publicKeyMultibase;
          expect(vm1mb, isNot(vm2mb));
      ...
      test(
        ...
        () async {
          ...
          final thumbprints = results
              .map((r) => r.verificationMethodId)
              .toList();
    ...
    group('Different key types - multiple keys with shared relationships', () {
      ...
      test(
        ...
        () async {
          ...
          expect(
            edAuthVmId,
            edAssertVmId,
          ); // auth and assert share same ed25519 thumbprint
          expect(
            edAuthVmId,
            isNot(edKaVmId),
          ); // keyAgreement uses x25519 thumbprint
      ...
      test(
        '3 keys (ed25519 + p256 + secp256k1): all with authentication only',
        () async {
          final edKey = await wallet.generateKey(
            keyId: 'ed-a',
            keyType: KeyType.ed25519,
    ...
    group('One-to-one key to VM mapping', () {
      ...
      test(
        ...
        () async {
          ...
          expect(
            result.relationships[VerificationRelationship.capabilityDelegation],
            primaryVmId,
          );
